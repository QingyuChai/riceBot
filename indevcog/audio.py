...
class Audio:
    ...
    async def _add_song_status(self, song):
        ...
        game = discord.Game(name="with FwiedWice :3")
        await self.bot.change_presence(status=status, game=game)
        log.debug('Song title: ' + song.title)
    ...
    async def _queue_list(self, ctx):
        ...
        if now_playing is not None:
            data = discord.Embed(description="**Queue**".format(now_playing.title), colour=discord.Colour(value=colour))
    ...
    async def song(self, ctx):
        ...
        if song:
            ...
            if hasattr(song, 'duration'):
                ...
                dur = None
            colour='099999'
            data = discord.Embed(description="**Song Info**", colour=discord.Colour(value=colour))
            data.add_field(name="**Title**", value=song.title, inline=False)
            ...
            data.add_field(name="Link", value=song.webpage_url, inline=False)
            data.set_footer(text="riceBot")
            await self.bot.say(embed=data)
