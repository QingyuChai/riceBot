...
class Account:
    ...
    @account.command(name="signup", pass_context=True, invoke_without_command=True, no_pm=True)
    ...
    @account.command(name="show", pass_context=True, invoke_without_command=True, no_pm=True)
    async def _acc(self, ctx, user : discord.Member=None):
        ...
        if not user:
            ...
            if user.id in self.nerdie:
                ...
                if "Age" in self.nerdie[user.id]:
                    age = self.nerdie[user.id]["Age"]
                    data.add_field(name="Age:", value=age)
                else:
                    pass
                if "Site" in self.nerdie[user.id]:
                    site = self.nerdie[user.id]["Site"]
                    data.add_field(name="Website:", value=site)
                else:
                    pass
                if "About" in self.nerdie[user.id]:
                    about = self.nerdie[user.id]["About"]
                    data.add_field(name="About:", value=about)
                else:
                    pass
                if "Gender" in self.nerdie[user.id]:
                    gender = self.nerdie[user.id]["Gender"]
                    data.add_field(name="Gender:", value=gender)
                else:
                    pass 
                if "Job" in self.nerdie[user.id]:
                    job = self.nerdie[user.id]["Job"]
                    data.add_field(name="Profession:", value=job)
                else:
                    pass
                if "Email" in self.nerdie[user.id]:
                    email = self.nerdie[user.id]["Email"]
                    data.add_field(name="Email Address:", value=email)
                else:
                    pass
                if "Other" in self.nerdie[user.id]:
                    other = self.nerdie[user.id]["Other"]
    ...
    @account.group(name="update", pass_context=True, invoke_without_command=True, no_pm=True)
    ...
    async def about(self, ctx, *, about):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"About" : about})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def website(self, ctx, *, site):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"Site" : site})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def age(self, ctx, *, age):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"Age" : age})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def job(self, ctx, *, job):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"Job" : job})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def gender(self, ctx, *, gender):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"Gender" : gender})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def email(self, ctx, *, email):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"email" : email})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
    ...
    async def other(self, ctx, *, other):
        ...
        if user.id not in self.nerdie:
            ...
            self.nerdie[user.id].update({"Other" : other})
            dataIO.save_json(self.profile, self.nerdie)
            data = discord.Embed(colour=user.colour)
